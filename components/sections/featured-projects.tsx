...
import type { ProjectCard as ProjectCardType } from "@/types/sanity/projects";
...
export function FeaturedProjects({ projects }: FeaturedProjectsProps) {
  ...
  return (
    <SectionWrapper bg="primary-50" maxWidth="wide">
      {/* Hero Landscape Block */}
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.2 }}
        transition={{ duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] }}
        className="relative w-full aspect-[4/3] sm:aspect-[16/9] md:aspect-[21/9] min-h-[280px] sm:min-h-[320px] md:min-h-[360px] lg:min-h-[400px] rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl overflow-hidden shadow-[0_8px_40px_-12px_rgba(27,58,45,0.2)] mb-8 sm:mb-10 md:mb-16"
      >
        <motion.div
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
        {featuredProjects.length === 0 ? null : featuredProjects.map((project) => (
          <ProjectCard
            key={project.id}
            image={project.mainImage?.asset?.url ?? "/images/project-3.jpg"}
            slug={project.slug}
            title={project.title}
            description={project.shortDescription}
            location={project.category ?? project.clientName ?? ""}
